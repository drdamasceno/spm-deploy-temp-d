...
def conciliar(
    ...
    for pp in pp_data:
        ...
        if not candidatos:
            # Nenhum candidato por nome — registro fica NAO_CLASSIFICADO.
            # Motor NÃO casa PIX por valor coincidente sem titularidade
            # (regra docstring linha 22). Casar por valor sem titular cria
            # falso positivo cross-prestador (ex: PIX da Eduarda Vitor R$ 10.980
            # sendo atribuído ao Henrique Storino pelo valor exato), e populava
            # registro_pp_id em transacao_bancaria — exibido como pago em
            # /contratos com risco real de pagamento duplicado pela Thais.
